...
namespace FTN.ESI.SIMES.CIM.CIMAdapter.Importer
{
	...
	public class PowerTransformerImporter
	{
		...
		private ResourceDescription CreateRegularTimePointResourceDescription(FTN.RegularTimePoint cimRegularTimePoint)
		{
			...
			{
				long gid = ModelCodeHelper.CreateGlobalId(0, (short)DMSType.REGTIMEPOINT, importHelper.CheckOutIndexForDMSType(DMSType.REGTIMEPOINT));
				rd = new ResourceDescription(gid);
		...
		private void ImportBreakers()
		{
			...
			if (cimBreakers != null)
			{
				foreach (KeyValuePair<string, object> cimBreakersPair in cimBreakers)
				{
					FTN.Breaker cimBreaker = cimBreakersPair.Value as FTN.Breaker;
        ...
        private ResourceDescription CreateLoadBreakSwitchResourceDescription(FTN.LoadBreakSwitch cimLoadBreakSwitch)
        {
            ...
            if (cimLoadBreakSwitch != null)
            {
                ...
                ////populate ResourceDescription
                PowerTransformerConverter.PopulateLoadBreakSwitchProperties(cimLoadBreakSwitch, rd, importHelper, report);
            }
            return rd;
        }
